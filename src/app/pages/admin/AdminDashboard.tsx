...
import { Users, ShoppingBag, DollarSign, Activity, TrendingUp, BarChart3, Eye, Flag, Star } from 'lucide-react';
...
type DashboardStats = {
    ...
    trendData: { date: string; ads: number; views: number }[];
    ...
    useEffect(() => {
        async function fetchStats() {
            try {
                ...
                const featuredAds = safeAds.filter((ad) => Boolean(ad.featured)).length;
...
function StatCard({ title, value, icon, tint }: { title: string; value: string; icon: ReactNode; tint: 'blue' | 'green' | 'purple' | 'indigo' | 'emerald' | 'yellow' | 'red' }) {
    const tintClass = {
