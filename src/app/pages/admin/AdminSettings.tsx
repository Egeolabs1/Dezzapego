            ...
            <div className="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <section className="border-t border-gray-100 pt-6">
                    ...
                    <div className="mt-4 space-y-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <div className="rounded-lg border border-gray-100 bg-gray-50 p-4"><label htmlFor="announcement-repeat-count" className="font-medium text-gray-900">Mensagens simultâneas: {globalAnnouncement.repeatCount}</label><input id="announcement-repeat-count" type="range" min="1" max="6" step="1" disabled={!globalAnnouncement.scroll} className="mt-3 w-full accent-blue-600 disabled:cursor-not-allowed disabled:opacity-50" value={globalAnnouncement.repeatCount} onChange={(event) => updateAnnouncement({ repeatCount: Number(event.target.value) })} /><p className="mt-1 text-xs text-gray-500">Com 1, uma mensagem entra novamente apenas após sair da tela.</p></div>
                            <div className="rounded-lg border border-gray-100 bg-gray-50 p-4"><label htmlFor="announcement-gap" className="font-medium text-gray-900">Distância entre mensagens: {globalAnnouncement.gap}px</label><input id="announcement-gap" type="range" min="0" max="200" step="4" disabled={!globalAnnouncement.scroll || globalAnnouncement.repeatCount === 1} className="mt-3 w-full accent-blue-600 disabled:cursor-not-allowed disabled:opacity-50" value={globalAnnouncement.gap} onChange={(event) => updateAnnouncement({ gap: Number(event.target.value) })} /><p className="mt-1 text-xs text-gray-500">Use 0px para mensagens encostadas.</p></div>
                        </div>
                        <div className="grid gap-4 md:grid-cols-2">
